...
//        if (locBitmap == null) {
//            locBitmap = BitmapFactory.decodeResource(context.getResources(), R.mipmap.ic_blue_mark);
//        }
    }
...
//            style.addLayer(symbolLayer);
    ...
    public void setMapReadyListener(MapReadyListener listener) {
        ...
        if (mapView != null) {
            mapView.getMapAsync(this::initReadyMapBox);
        }
    }
    ...
    private void initReadyMapBox(MapboxMap mapboxMap) {
        ...
        mapboxMap.setStyle(MapType.valueOf(MapType.DEFAULT.name()).toString(), style -> {
            mapStyle = style;
            mapStyleLoaded = true;
...
//        });
        ...
        mapboxMap.addOnMapLongClickListener(latLng -> {
            if (mapLongClickListener != null) {
                mapLongClickListener.OnMapLongClickListener(new com.wz.location.map.model.LatLng(latLng.getLatitude(), latLng.getLongitude()));
            }
            return false;
        });
        ...
        mapboxMap.addOnCameraMoveStartedListener(i -> {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (curMoveMark != null) {
                    ...
                    if (curTimeStamp - lastClickTime >= DRAG_TIME || DistanceUtil.distance(this.context, pressedX, pressedY, event.getX(), event.getY()) >= MAX_CLICK_DISTANCE) { // 拖动
                        ...
                        // 将像素点转换为坐标
                        com.wz.location.map.model.LatLng curPoint = mMarkManager.fromScreenPoint(tapPoint);
     ...
     * @return 返回添加mark
     */
    public Marker addMarker(MarkerOptions markerOptions, String id) {
        if (markerOptions == null) {
            return null;
        }
        if (mMarkManager == null) {
        ...
        return mMarkManager.addMark(markerOptions, id);
    }
     ...
     * @return 返回添加圆
     */
    public final Circle addCircle(CircleOptions circleOptions, String id) {
        if (circleOptions == null) {
            return null;
        }
        if (mLayerManager == null) {
     ...
     * @return 经纬度
     */
    public com.wz.location.map.model.LatLng getCenterLocation() {
        if (mapboxMap == null) {
            return null;
        }
        ...
        return new com.wz.location.map.model.LatLng(cameraPosition.target.getLatitude(), cameraPosition.target.getLongitude());
    }
}
